...
class OutputPanel(Vertical):
    ...
    def add_command_result(self, command: str, result: str) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to add command result to output panel: {e}")
    ...
    def add_ai_response(self, query: str, response: str, streaming: bool = False) -> None:
        ...
        try:
            ...
            # Fallback to logging if widget isn't ready
            import logging
            logger = logging.getLogger("k2edit")
            logger.warning(f"Failed to add AI response to output panel: {e}")
    ...
    def add_error(self, error_message: str) -> None:
        ...
        try:
            ...
            import logging
            logger = logging.getLogger("k2edit")
            logger.warning(f"Failed to add error to output panel: {e}")
    ...
    def add_info(self, info_message: str) -> None:
        ...
        try:
            ...
            import logging
            logger = logging.getLogger("k2edit")
            logger.warning(f"Failed to add info to output panel: {e}")
            logger.info(f"Info message: {info_message}")
    ...
    def add_tool_execution(self, tool_name: str, arguments: dict, result: str = None) -> None:
        ...
        try:
            ...
            import logging
            logger = logging.getLogger("k2edit")
            logger.warning(f"Failed to add tool execution to output panel: {e}")
            logger.info(f"Tool: {tool_name}, Arguments: {arguments}, Result: {result}")
    ...
    def _display_content(self, content: str) -> None:
        ...
        try:
            ...
            import logging
            logger = logging.getLogger("k2edit")
            logger.warning(f"Failed to display content: {e}")
            try:
                log = self.query_one("#output-log", RichLog)
                if log:
                    log.write(content)
            except:
                logger.error(f"Failed to write content: {content}")
    ...
    def _display_markdown(self, content: str) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to write markdown content: {content}")
    ...
    def _display_code(self, content: str, language: str = "text") -> None:
        ...
        try:
            ...
            try:
                log = self.query_one("#output-log", RichLog)
                if log:
                    log.write(f"```{language}\n{content}\n```")
            except:
                logger.error(f"Failed to write code: {content}")
    ...
    def add_agent_progress(self, request_id: str, current_iteration: int, max_iterations: int = None, status: str = "processing") -> None:
        ...
        try:
            ...
            import logging
            logger = logging.getLogger("k2edit")
            logger.warning(f"Failed to add agent progress to output panel: {e}")
